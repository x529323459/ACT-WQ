...
class PResNet(nn.Module):
    def __init__(
        ...
        if pretrained:
            # 获取项目根目录，构建正确的预训练模型路径
            current_file = os.path.abspath(__file__)
            project_root = os.path.dirname(os.path.dirname(os.path.dirname(current_file)))
            
            if depth == 50:
                model_path = os.path.join(project_root, 'pre_model/ResNet50_vd_ssld_v2_pretrained_from_paddle.pth')
                state = torch.load(model_path)
            elif depth == 18:
                model_path = os.path.join(project_root, 'pre_model/ResNet18_vd_pretrained_from_paddle.pth')
                state = torch.load(model_path)
            else:
