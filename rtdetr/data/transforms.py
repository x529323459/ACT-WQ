        ...
        if ops is not None:
            for op in ops:
                if isinstance(op, dict):
                    ...
                    # 改写hy
                    if name == 'RandomIoUCrop':
                        transfom = RandomIoUCrop(**op)
                    elif name == 'ConvertBox':
                        transfom = ConvertBox(**op)
                    else:
                        transform_class = TRANSFORM_CLASSES.get(name)
                        if transform_class is None:
                            raise ValueError(f"Unknown transform type: {name}")
                        transfom = transform_class(**op)
