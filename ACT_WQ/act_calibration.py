...
def _flatten_out(x: Any) -> torch.Tensor:
    if isinstance(x, torch.Tensor):
        ...
        return torch.cat(vals) if len(vals) else torch.tensor(0.0, device=next(iter(x.values())).device)
    elif isinstance(x, (list, tuple)):
        vals = [_flatten_out(v) for v in x if isinstance(v, (torch.Tensor, dict, list, tuple))]
        if len(vals) == 0:
            return torch.tensor(0.0)
        dev = vals[0].device
        return torch.cat(vals) if len(vals) else torch.tensor(0.0, device=dev)
    return torch.tensor(0.0)
...
def _get_ch_axis(mod: nn.Module, x: torch.Tensor) -> int:
    # Conv2d: NCHW -> 1; Linear: [B, C] or [B, T, C] -> last dim
        ...
        def hook(mod, inputs, output):
            ...
            for c in ch_ids:
                ...
                if kk <= 0:
                    ...
                    continue
                idx = torch.randint(0, nnv, (kk,), device=x.device)
...
def _grid_search_clip_channelwise(
    xs_per_ch: List[torch.Tensor],
    H_per_ch: torch.Tensor,
    bit: int,
    percentiles=(0.999, 0.9995, 0.9997, 0.9999, 0.99995, 0.99999),
...
def calibrate_gptq_activations(
    fp_model: nn.Module,
    q_model: nn.Module,
    dataloader,
    act_bits: int = 6,
